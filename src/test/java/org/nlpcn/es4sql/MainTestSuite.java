...
//        System.out.println("Finished the setup process...");
    ...
    private static void preparePhrasesIndex() {
        String dataMapping = "{  \"phrase\": {" + " \"properties\": {\n" + "          \"phrase\": {\n"
                + "            \"type\": \"string\",\n" + "            \"store\": true\n" + "          }" + "       }"
                + "   }" + "}";
        client.admin().indices().preparePutMapping(TEST_INDEX).setType("phrase").setSource(dataMapping).execute()
                .actionGet();
    }
    ...
    private static void prepareNestedTypeIndex() {
        ...
        client.admin().indices().preparePutMapping(TEST_INDEX).setType("nestedType").setSource(dataMapping).execute()
                .actionGet();
    }
    ...
    private static void prepareChildrenTypeIndex() {
        ...
        client.admin().indices().preparePutMapping(TEST_INDEX).setType("childrenType").setSource(dataMapping).execute()
                .actionGet();
    }
    ...
    private static void prepareParentTypeIndex() {
        ...
        client.admin().indices().preparePutMapping(TEST_INDEX).setType("parentType").setSource(dataMapping).execute()
                .actionGet();
    }
    ...
    public static void loadBulk(String jsonPath) throws Exception {
        ...
        if (response.hasFailures()) {
            throw new Exception(String.format("Failed during bulk load of file %s. failure message: %s", jsonPath,
    ...
    public static void prepareSpatialIndex(String type) {
        ...
        client.admin().indices().preparePutMapping(TEST_INDEX).setType(type).setSource(dataMapping).execute()
                .actionGet();
    }
    ...
    public static void prepareOdbcIndex() {
        ...
        client.admin().indices().preparePutMapping(TEST_INDEX).setType("odbc").setSource(dataMapping).execute()
                .actionGet();
    }
