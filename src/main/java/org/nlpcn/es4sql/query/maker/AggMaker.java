...
public class AggMaker {
    ...
    private AggregationBuilder<?> makeRangeGroup(MethodField field) throws SqlParseException {
        ...
        switch (field.getName().toLowerCase()) {
            case "range":
                aggregationBuilder = rangeBuilder(field);
                break;
            case "date_histogram":
                aggregationBuilder = dateHistogram(field);
                break;
            case "date_range":
                aggregationBuilder = dateRange(field);
                break;
            case "month":
                aggregationBuilder = dateRange(field);
                break;
            case "histogram":
                aggregationBuilder = histogram(field);
                break;
            case "geohash_grid":
                aggregationBuilder = geohashGrid(field);
                break;
            case "geo_bounds":
                aggregationBuilder = geoBounds(field);
                break;
            case "terms":
                aggregationBuilder = termsAgg(field);
                break;
            default:
