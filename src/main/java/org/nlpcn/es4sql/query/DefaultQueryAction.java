...
public class DefaultQueryAction extends QueryAction {
    ...
    private void setSorts(List<Order> orderBys) {
        for (Order order : orderBys) {
            MethodField methodField = getOrderScript(order.getName(), select);
            if (methodField != null&&methodField.getName().equals("script")) {
                ScriptSortBuilder sort = new ScriptSortBuilder(new Script(methodField.getParams().get(1).value.toString()), "string");
                sort.order(order.getSortOrder());
                request.addSort(sort);
            } else if(order.getScript()!=null) {
    ...
    private MethodField getOrderScript(String name, Select select) {
